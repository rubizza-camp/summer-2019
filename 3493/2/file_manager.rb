class FileManager
  ...
  def self.save_location_in_file(location, person_number, folder_name)
    location_path = "#{person_number}/#{folder_name}/#{Date.today}/#{SecureRandom.hex(4)}"
  ...
  def save_photo_from_uri(path, person_number, folder_name)
    ...
    photo_new_path = "#{person_number}/#{folder_name}/#{Date.today}/#{SecureRandom.hex(4)}.jpg"
    File.write(photo_new_path, open(uri).read, {mode: 'wb'})
    return photo_new_path
  end
