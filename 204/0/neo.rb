...
module Neo
  ...
  class Sensei
    ...
    def puts_message
      if (recents = progress.last(5)) && recents.count == 5 && recents.uniq.count == 1
        puts Color.cyan('  I sense frustration. Do not be afraid to ask for help.')
      elsif progress.last(2).count == 2 && progress.last(2).uniq.count == 1
  ...
  class Koan
    ...
    class << self
      ...
      # :reek:UncommunicativeVariableName
      def total_tests
        subclasses.inject(0) { |total, k| total + k.testmethods.count }
      end
    end
  end
  # :reek:FeatureEnvy
