...
class TopRubyGems
	...
	def get_params_from_term
		unless ARGV == nil
  	  	...
  	  	opts.on("-v", "--name [STRING]", String, "Run verbosely") { |args| options[:raz] = args.strip.chomp }
				opts.on('-t', '--top= [STRING]', String, "Run verbose") { |args| options[:dva] = args.strip.chomp }
				opts.on('-f', '--file= [INTEGER[', Integer, "Run verbose") { |args| options[:tri] = args.strip.chomp }
			end
  		optparse.parse!
  		options
  	end
	...
	def get_data_from_yaml_to_hash
		@list_of_gems_in_yml = YAML.load(File.read('data/gems.yml'))
	...
	def show_top_ruby_gems
		show_greeting
		@rows = []
		@total_data_about_gem.each do |key, value|
		@rows << ["#{key}" ,
...
start_programm.show_top_ruby_gems