...
module Neo
  ...
  class Koan
    ...
    attr_reader :name, :failure, :koan_count, :step_c, :koan_file

    def initialize(name, koan_file = nil, koan_count = 0, step_c = 0)
      @name = name
      @failure = nil
    ...
    class << self
      ...
      def method_added(name)
        testmethods << name if !tests_disabled? &&
    ...
    def each_step
        ...
        Neo::Koan.subclasses.each_with_index do |koan, koan_index|
          koan.testmethods.each do |method_name|
            step = koan.new(method_name, koan.to_s, koan_index + 1, step_c += 1)
            yield step
          end
        end
      end
